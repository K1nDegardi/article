...
import { Check, CopyIcon, CornerDownLeft } from "lucide-react"
    ...
    <section className="mt-16 w-full max-w-xl">
        <div className="flex flex-col w-full gap-2">
            ...
            onSubmit={handleSubmit}>
                ...
                <button type="submit"
                className="submit_btn border-none  peer-focus:text-gray-700"
                >
                    <CornerDownLeft />                 
                </button>
            </form>
            ...
            <div className="flex flex-col gap-1 max-h-60 overflow-y-auto">
                {allArticles.map((item,index)=>(
                    <div key={`link-${index}`}
                        ...
                        className="link_card"
                    >
                        <div className="copy_btn mx-1 my-1" onClick={() => handleCopy(item.url)}>
                            {copied === item.url ? (
                                <Check />
                            ) : (
                                <CopyIcon
